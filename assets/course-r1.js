(() => {
  ...
  const STATE_KEY = 'ebgCourseR1State';
  const VERSION = 'R1-2026-08-30';
  const PASS_SCORE = 7;
  ...
  const I18N = {
    pl: {
      ...
      resetConfirm:'Wyzerować cały postęp kursu R1?', search:'Szukaj w kursie...', noResults:'Brak wyników w planie kursu.',
      finalDone:'Kurs ukończony', finalInProgress:'Kurs w toku', finalText:(l,p)=>`Lekcje: ${l}/45. Zaliczone testy: ${p}/5.`,
    ...
    en: {
      ...
      resetConfirm:'Reset all R1 course progress?', search:'Search the course...', noResults:'No results in the course plan.',
      finalDone:'Course completed', finalInProgress:'Course in progress', finalText:(l,p)=>`Lessons: ${l}/45. Passed quizzes: ${p}/5.`,
  ...
  function renderSidebar(){
    ...
    $$('.course-module').forEach((article,i)=>{
      ...
      button.addEventListener('click',()=>article.scrollIntoView({behavior:'smooth',block:'start'}));
      ...
      article.querySelectorAll(':scope > .required-section').forEach(sec=>{
        const a=document.createElement('a'); a.href='#'+sec.id; a.dataset.target=sec.id; a.textContent=sectionTitle(sec); if(state.completed.includes(sec.id)) a.classList.add('is-complete'); list.appendChild(a);
      });
      const qa=document.createElement('a'); qa.href='#quiz-'+article.dataset.module; qa.dataset.target='quiz-'+article.dataset.module; qa.textContent=t().quiz; if(state.quizzes[article.dataset.module]?.passed) qa.classList.add('is-complete'); list.appendChild(qa);
  ...
  function eligible(){return completedCount()===45&&passedCount()===5;}
  function coursePercent(){return Math.round(((completedCount()+passedCount())/50)*100);}
  ...
  function initNavigation(){
    $('#startCourse')?.addEventListener('click',()=>$('#r1-m1-1')?.scrollIntoView({behavior:'smooth',block:'start'}));
    $('#continueCourse')?.addEventListener('click',()=>{
      const first=$$('.required-section').find(s=>!state.completed.includes(s.id)); if(first)return first.scrollIntoView({behavior:'smooth',block:'start'});
      const q=MODULES.find(m=>!state.quizzes[m]?.passed); if(q)return $('#quiz-'+q)?.scrollIntoView({behavior:'smooth',block:'start'}); $('#courseFinal')?.scrollIntoView({behavior:'smooth'});
    });
    $$('[data-module-nav]').forEach(btn=>btn.addEventListener('click',()=>{
      const article=btn.closest('.course-module'), all=$$('.course-module'), i=all.indexOf(article), dir=btn.dataset.moduleNav==='next'?1:-1, target=all[i+dir]; if(target)target.scrollIntoView({behavior:'smooth',block:'start'}); else if(dir>0)$('#courseFinal')?.scrollIntoView({behavior:'smooth'});
    }));
    ...
    $('.sidebar-open')?.addEventListener('click',open); $('.sidebar-close')?.addEventListener('click',close); backdrop?.addEventListener('click',close); $('#sidebarNav')?.addEventListener('click',e=>{if(e.target.closest('a')&&innerWidth<=900)close()});
  }
  ...
  function initExportImport(){
    $('#exportProgress')?.addEventListener('click',()=>{ const blob=new Blob([JSON.stringify(state,null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`edukacja-bez-granic-kurs-r1-postep-${new Date().toISOString().slice(0,10)}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);});
