(() => {
  ...
  const messages = {
    real: {
      className: 'correct',
      title: 'Trafna ocena: profil jest raczej autentyczny.',
      body: 'Wskazuje na to wieloletnia i zróżnicowana aktywność, lokalne szczegóły, powracające relacje rodzinne oraz naturalne rozmowy z osobami, które znają autora poza internetem. Treści polityczne są wyraźne, ale nie stanowią całej tożsamości konta. Pamiętaj: autentyczny użytkownik również może udostępnić materiał błędny lub zmanipulowany.'
    },
    troll: {
      className: 'incorrect',
      title: 'W tym ćwiczeniu profil jest raczej autentyczny.',
      body: 'Duża liczba wpisów politycznych może wzbudzać podejrzenie, ale sama w sobie nie świadczy o operacji wpływu. Tu ważniejsze są: ciągłość prywatnej historii, konkretne lokalne doświadczenia, wzajemne relacje w komentarzach, różnorodność tematów i gotowość do doprecyzowania własnego stanowiska.'
    }
  };
  ...
  function choose(answer) {
    ...
    result.innerHTML = `<strong>${message.title}</strong><p>${message.body}</p><a href="#podsumowanie">Przejdź do omówienia ↓</a>`;
    result.hidden = false;
