(() => {
'use strict';
const COURSE_KEY='ebgCourseV2State',QUIZ_KEY='ebgCourseModuleQuizV1',LEGACY_QUIZ_KEY='ebgModuleQuizStateV1',PROFILE_KEY='ebgCourseV2Profile';
const TITLES={1:'Edukacja medialna w epoce cyfrowej',2:'Fake news, manipulacja i stronniczość',3:'Krytyczne myślenie i podejmowanie decyzji',4:'Tematy społecznie wrażliwe i wpływ mediów',5:'Odporność informacyjna, dobrostan i zaangażowanie demokratyczne'};
...
function requirements(){const stored=new Set(read(COURSE_KEY,{completed:[]}).completed||[]),sections=$$('.course-section[data-course-section]').filter(s=>s.dataset.quizRequired!=='false'),q={...read(LEGACY_QUIZ_KEY,{}),...read(QUIZ_KEY,{})};const isDone=s=>{const id=s.dataset.courseSection,nav=id?$(`[data-section-link="${CSS.escape(id)}"]`):null;return Boolean(id&&(stored.has(id)||s.classList.contains('is-complete')||nav?.classList.contains('is-complete')))};const done=sections.filter(isDone).length;const passed=[1,2,3,4,5].filter(m=>q[m]?.passed&&Number(q[m].score)>=7);return{done:passed.length===5?sections.length:done,total:sections.length,passed,q,eligible:passed.length===5}}
